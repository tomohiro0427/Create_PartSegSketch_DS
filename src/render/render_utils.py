...
class Render:
    ...
    def render(self, path, clean=True, intensity=10.0, mesh=None, only_render_images=False,points=False):
        if points:
            mesh = trimesh.points.PointCloud(mesh)
            normalize_mesh(mesh, "com")
            mesh1 = pyrender.Mesh.from_points(mesh.vertices)
        else :
...
def prepare_mesh(model_name, color=False, clean=False):
    # mesh = trimesh.load(model_name, force="mesh", skip_materials=True)
    o3d.utility.set_verbosity_level(o3d.utility.VerbosityLevel.Error)
    mesh = o3d.io.read_triangle_mesh(str(model_name))
...
def pyrender_rendering(mesh, camera_pose, viz=False, light=False, intensity=10.0, bg_color=None, yfov=np.pi / 3.0):
