...
class DGCNN_partseg(nn.Module):
    def __init__(self, seg_num_all):
        ...
        self.conv5 = nn.Sequential(nn.Conv2d(64*2, 64, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv6 = nn.Sequential(nn.Conv1d(192, 512, kernel_size=1, bias=False),
                                   self.bn6,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv7 = nn.Sequential(nn.Conv1d(1280, 256, kernel_size=1, bias=False),
                                   self.bn7,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv8 = nn.Sequential(nn.Conv1d(512+256+64*3, 256, kernel_size=1, bias=False),
                                   self.bn8,
                                   nn.LeakyReLU(negative_slope=0.2))
    ...
    def forward(self, x, image_features):
        ...
        l = image_features.view(batch_size, -1, 1)           # (batch_size, num_categoties, 1)
        l = self.conv7(l)                       # (batch_size, 1280, 1) -> (batch_size, 256, 1)

        x = torch.cat((x, l), dim=1)            # (batch_size, 512+256, 1)
        x = x.repeat(1, 1, num_points)          # (batch_size, 512+256, num_points)
        # print(x.shape)
        # print(x1.shape)
        # print(x2.shape)
        # print(x3.shape)

        x = torch.cat((x, x1, x2, x3), dim=1)   # (batch_size, 512+256+64*3, num_points)

        x = self.conv8(x)                       # (batch_size, 512+256+64*3, num_points) -> (batch_size, 256, num_points)
        x = self.dp1(x)
